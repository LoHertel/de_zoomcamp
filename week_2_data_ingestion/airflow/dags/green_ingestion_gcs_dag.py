...
FILE_NAME = 'green_tripdata_{{ dag_run.logical_date | iso_month }}.csv'
...
with DAG(
    dag_id="green_tripdata_ingestion_gcs_dag",
    description="Download monthly New York green taxi trip data and ingest it to BigQuery.",
    ...
    remove_local_dataset_task = BashOperator(
        task_id="remove_local_dataset",
        #bash_command=f"curl -sS {dataset_url} > {path_to_local_home}/{dataset_file}"
        bash_command=f'rm {TARGET_FOLDER}/{TARGET_FILE} {TARGET_FOLDER}/{TARGET_PARQUET}'
    )



    download_dataset_task >> format_to_parquet_task >> local_to_gcs_task >> remove_local_dataset_task